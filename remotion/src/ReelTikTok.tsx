...
const SceneVoice: React.FC<{ vo: SceneVo | null | undefined; dark?: boolean; captions?: boolean; align?: 'center' | 'left'; raise?: number }> = ({ vo, dark, captions = true, align = 'center', raise = 0 }) => {
	const frame = useCurrentFrame();
	if (!vo) return null;
	// GANZE-SÄTZE-Segmentierung (Aaron 2026-07-19: „warum geben wir dem Sprecher
	// Stichpunkte statt Text — wirkt verbuggt"). Ein Satz = eine Caption: NUR an echten
	// Satz-Enden (. ! ? → sbrk) brechen, NICHT an Komma/Semikolon. Erst wenn ein Satz
	// länger als MAX_W Wörter würde, an einem Satzteil (brk) notbrechen, damit die Pille
	// nie über den Rand läuft. So liest sich die Caption als Satz, nicht als Häppchen.
	const MAX_W = 8;
	const chunks: CaptionWord[][] = [];
	let cur: CaptionWord[] = [];
	for (const w of vo.words) {
		cur.push(w);
		const endsSentence = w.sbrk || /[.!?]$/.test(w.t.trim());
		const tooLong = cur.length >= MAX_W && (w.brk || /[.,;:!?–—]$/.test(w.t.trim()));
	...
	// Verwaiste Mini-Pills („Erstmals." allein nach langem Satz) wirken wie ein Render-Bug
	// → an den vorigen Chunk hängen, sofern der dadurch nicht über MAX_W+2 läuft.
	if (chunks.length >= 2 && chunks[chunks.length - 1].length < 2 && chunks[chunks.length - 2].length <= MAX_W) {
...
export const ReelTikTok: React.FC<ReelDailyProps> = (p) => {
	...
	return (
		<AbsoluteFill style={{ background: CANVAS }}>
			{/* MUSIK-BETT: seit 2026-08-01 standardmaessig AUS (Aaron). Der Sound kommt beim
			    Posten aus TikToks Commercial Music Library — ein Musikbett im Master wuerde
			    sich damit ueberlagern. Der Baustein bleibt erhalten (Ducking, Cold-Open-Stille)
			    und laesst sich per plan.music bzw. musicFile jederzeit reaktivieren. */}
			{p.musicFile ? (
			<Audio
