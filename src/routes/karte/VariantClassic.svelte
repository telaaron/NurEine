<script lang="ts">
	...
	import { ChevronRightIcon, GlobeAltIcon } from 'heroicons-svelte/24/outline';
	...
	let { stories: storiesProp = [] }: { stories?: StoryMarker[] } = $props();

	const stories = $derived(storiesProp as StoryMarker[]);
			...
			{#if activeStory}
				...
				>
					...
					<div class="p-5 sm:p-6 lg:p-7">
						...
						>
							Geschichte lesen
							<Icon icon={ChevronRightIcon} size="1rem" />
						</a>
					</div>
				</div>
			{:else}
				<div
					...
					style="border: 1px solid var(--color-rule);"
				>
					<div class="text-3xl" style="color: var(--color-muted);">
						<Icon icon={GlobeAltIcon} size="1.875rem" />
					</div>
